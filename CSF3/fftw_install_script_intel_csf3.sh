...
./configure --prefix=$APPDIR --enable-float --enable-sse2 --enable-openmp --enable-threads 2>&1 | tee ../config-$APPVER.log
...
./configure --prefix=$APPDIR --enable-sse2 --enable-openmp --enable-threads 2>&1 | tee ../config-$APPVER.log
make 2>&1 | tee make-$APPVER.log
make check 2>&1 | tee make-check-$APPVER.log
make install 2>&1 | tee make-install-$APPVER.log
...
MDIR=/mnt/iusers01/support/mbessdl2/privatemodules/priv_libs/intel/fftw
