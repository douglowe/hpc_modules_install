...
# Note: if compute time is exceeded, this command can be resubmitted, and
#       the check process will be picked up from the last run check
qrsh -l short -V -cwd -pe smp.pe 12 make check 2>&1 | tee make-check-$APPVER.log
#qsub -V -cwd -pe smp.pe 12 -b y make check
...
MPATH=libs/gcc/hdf5/${APPVER}_mpi
...
module load mpi/\${COMPDIR}/openmpi/4.1.2-gcc-8.2.0
