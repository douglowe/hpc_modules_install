...
APPROOT=/mnt/iusers01/support/mbessdl2/privatemodules_packages/csf3/libs/gcc/paradyn
...
#module load libs/gcc/tbb/2021.4



wget https://github.com/dyninst/dyninst/archive/refs/tags/v${APPVER}.tar.gz

cd ../build
tar zxf ../archive/v${APPVER}.tar.gz

mkdir dyninst-${APPVER}-build
cd dyninst-${APPVER}-build

# Note: findboost seems incapable of expanding environment variables, so use double quotes 
#       here to make sure that the shell does this first
BOOST_FLAGS="-DPATH_BOOST=$BOOSTDIR"
ENV_FLAGS="-DLibElf_INCLUDE_DIR=/usr/lib64/elfutils -DLibDwarf_INCLUDE_DIR=/usr/lib64 -DTBB_ROOT_DIR=$TBBDIR"
ENV_FLAGS="-DTBB_ROOT_DIR=$TBBDIR"

cmake ../dyninst-${APPVER} -DCMAKE_INSTALL_PREFIX=$APPDIR  $BOOST_FLAGS $ENV_FLAGS | tee ../config-cmake-$APPVER.log

make VERBOSE=1 2>&1 | tee make-$APPVER.log
